function [SPKF] = SPKF_attitude_crs(Sensor, SPKF, EKF, sensParams, tStep)
...
Q_k_1 = tStep/2*diag([sensParams.var_gyr-sensParams.var_bias_gyr/6*tStep^2;
                    sensParams.var_bias_gyr*.01]); 
...
for ii = 1:L
    ...
    %convert MRP sigma points into quaternions by crassidis' chose f = 2*(a+1)
...
for ii = 1:2*L+1
    ...
    omega_sig(:,ii) = u_b_gyr - Sigma_pts_k_1(4:6,ii); %angular velocity
...
%     q_k_m_sig(:,ii) = q_k_m_sig(:,ii)/norm(q_k_m_sig(:,ii)); %renorm just incase
    ...
    Sigma_pts_k_m(4:6,ii) = Sigma_pts_k_1(4:6,ii);
end
...
if ~SPKF.use_acc
    ...
    for ii = 1:2*L+1
        ...
        Sigma_Y(1:3,ii) = rotMat*(mag); %magnetometer        
    end
    ...
    for ii = 1:2*L+1
        ...
        Sigma_Y(1:3,ii) = rotMat*([0;0;g]) + bias_acc; %accel
        Sigma_Y(4:6,ii) = rotMat*(mag); %magnetometer
