function [ASPKF_opt_noN] = ASPKF_opt_attitude_noN(Sensor, ASPKF_opt_noN, AEKF, sensParams, tStep)
% ASPKF_opt_noN - kinematic prediction for orientation
...
% ASPKF opt with measurements not normalized
...
q_k_1 = ASPKF_opt_noN.X_hat.q_hat;

omega_k_1 = ASPKF_opt_noN.X_hat.omega_hat;

rotMat_k_1 = quat2rotmat(q_k_1);

%bias terms
% bias_acc = AEKF.X_hat.bias_acc;
bias_gyr = ASPKF_opt_noN.X_hat.bias_gyr;
...
Q_k_1_G = ASPKF_opt_noN.Lam_k + Q_k_1;


P_k_1_att =  ASPKF_opt_noN.P_hat;

%measurement noise adaptation
% ASPKF_opt_noN.use_acc(2:end) = ASPKF_opt_noN.use_acc(1:end-1); % shift accelerometer use vector

if norm(u_b_acc,2) > norm(g,2) + ASPKF_opt_noN.accel_bound || norm(u_b_acc,2) < norm(g,2) - ASPKF_opt_noN.accel_bound
    %sig for correct (measurements)        
    R_k = diag([sensParams.var_mag]);
    
    R_k_G = ASPKF_opt_noN.S_k(4:6,4:6) + R_k;
    
    ASPKF_opt_noN.use_acc(1) = 0;
else
    R_k = diag([sensParams.var_acc;
                sensParams.var_mag]);
    
    R_k_G = ASPKF_opt_noN.S_k + R_k;
    
    ASPKF_opt_noN.use_acc(1) = 1;
end
...
lambda = ASPKF_opt_noN.alpha^2*(L-ASPKF_opt_noN.kappa)-L;
...
P_k_m = (lambda/(L+lambda)+(1-ASPKF_opt_noN.alpha^2+ASPKF_opt_noN.beta))*((Sigma_pts_k_m(1:6,1)-X_k_m(1:6))*(Sigma_pts_k_m(1:6,1)-X_k_m(1:6))');
for ii = 2:2*L+1
...
V_k = (lambda/(L+lambda)+(1-ASPKF_opt_noN.alpha^2+ASPKF_opt_noN.beta))*(Sigma_Y(:,1)-y_k_hat)*(Sigma_Y(:,1)-y_k_hat)';
for ii = 2:2*L+1
    V_k = V_k+.5/(L+lambda)*(Sigma_Y(:,ii)-y_k_hat)*(Sigma_Y(:,ii)-y_k_hat)';
end
% V_k = V_k + R_k;

U_k = (lambda/(L+lambda)+(1-ASPKF_opt_noN.alpha^2+ASPKF_opt_noN.beta))*(Sigma_pts_k_m(1:6,1)-X_k_m(1:6))*(Sigma_Y(:,1)-y_k_hat)';
...
ASPKF_opt_noN.innov_error(:,:,2:end) = ASPKF_opt_noN.innov_error(:,:,1:end-1);
...
if  ASPKF_opt_noN.use_acc(1) == 1
    ...
    ASPKF_opt_noN.innov_error(:,:,1) = temp;
...
if sum(ASPKF_opt_noN.use_acc) ~= 0
    innov_sum(1:3,1:3) = sum(ASPKF_opt_noN.innov_error(1:3,1:3),3)/sum(ASPKF_opt_noN.use_acc);
    innov_sum(4:6,1:3) = sum(ASPKF_opt_noN.innov_error(4:6,1:3),3)/sum(ASPKF_opt_noN.use_acc);
    innov_sum(1:3,4:6) = sum(ASPKF_opt_noN.innov_error(1:3,4:6),3)/sum(ASPKF_opt_noN.use_acc);
else
    innov_sum = zeros(6);
end

innov_sum(4:6,4:6) = sum(ASPKF_opt_noN.innov_error(4:6,4:6),3)/ASPKF_opt_noN.moving_win;

ASPKF_opt_noN.S_k = innov_sum;


% % this was the old way of doing it. each old innov error was weighted the
% % same - no fading factor was used

% if  ASPKF_opt_noN.use_acc(1) == 1
% ASPKF_opt_noN.innov_error(:,:,1) = innov*innov'; % use full innovation matrix
% 
% else
% ASPKF_opt_noN.innov_error(:,:,1) = zeros(6); % only have magnetometer error in innovation matrix
% ASPKF_opt_noN.innov_error(4:6,4:6,1) = innov*innov'; 
% 
% end
% 
% %update each block of adaptive gain individually as there are not as many
...
% if ASPKF_opt_noN.use_acc(1) == 0
%     
%     innov_sum = sum(ASPKF_opt_noN.innov_error(4:6,4:6),3)/ASPKF_opt_noN.moving_win ;
%     ASPKF_opt_noN.S_k(4:6,4:6) = innov_sum - V_k;
%     
% else
%     innov_sum(1:3,1:3) = sum(ASPKF_opt_noN.innov_error(1:3,1:3),3)/sum(ASPKF_opt_noN.use_acc);
%     innov_sum(4:6,1:3) = sum(ASPKF_opt_noN.innov_error(4:6,1:3),3)/sum(ASPKF_opt_noN.use_acc);
...
%     
%     innov_sum(4:6,4:6) = sum(ASPKF_opt_noN.innov_error(4:6,4:6),3)/ASPKF_opt_noN.moving_win;
%     ASPKF_opt_noN.S_k = innov_sum - V_k;
% 
% end
...
S_k_tmp = diag(ASPKF_opt_noN.S_k);
for ii = 1:length(S_k_tmp)
    if S_k_tmp(ii)< 0
        S_k_tmp(ii) = 0;
    end
end
ASPKF_opt_noN.S_k = diag(S_k_tmp);
...
% temp = P_k_m\U_k;
% 
% ASPKF_opt_noN.Lam_k = temp'\(innov_sum - R_k)/temp - P_k_m;
% 
% % only use diagonal terms of gain and set to 0 if less than 0
% Lam_k_tmp = diag(ASPKF_opt_noN.Lam_k);
% for ii = 1:length(Lam_k_tmp)
%     if Lam_k_tmp(ii)< 0
%         Lam_k_tmp(ii) = 0;
%     end
% end
% ASPKF_opt_noN.Lam_k = diag(Lam_k_tmp);



% update 
ASPKF_opt_noN.X_hat.bias_gyr = X_k_m(4:6) + DX_k(4:6);

ASPKF_opt_noN.X_hat.omega_hat = u_b_gyr - ASPKF_opt_noN.X_hat.bias_gyr; %ang vel is just gyro minus bias
...
ASPKF_opt_noN.X_hat.q_hat = quatmultiply(q_k_upd,q_k_m_sig(:,1));
...
ASPKF_opt_noN.P_hat = 0.5*(P_hat + P_hat');
