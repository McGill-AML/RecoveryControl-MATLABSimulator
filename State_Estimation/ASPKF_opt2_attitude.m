function [ASPKF_opt2] = ASPKF_opt2_attitude(Sensor, ASPKF_opt2, AEKF, sensParams, tStep)
% ASPKF_opt2 - kinematic prediction for orientation
...
q_k_1 = ASPKF_opt2.X_hat.q_hat;

omega_k_1 = ASPKF_opt2.X_hat.omega_hat;

rotMat_k_1 = quat2rotmat(q_k_1);

%bias terms
% bias_acc = AEKF.X_hat.bias_acc;
bias_gyr = ASPKF_opt2.X_hat.bias_gyr;
...
P_k_1_att =  ASPKF_opt2.P_hat;

%measurement noise adaptation
ASPKF_opt2.use_acc(2:end) = ASPKF_opt2.use_acc(1:end-1); % shift accelerometer use vector

if norm(u_b_acc,2) > norm(g,2) + ASPKF_opt2.accel_bound || norm(u_b_acc,2) < norm(g,2) - ASPKF_opt2.accel_bound
    %sig for correct (measurements)        
    R_k = diag([sensParams.var_mag]);
    
    
    ASPKF_opt2.use_acc = 0;
else
    R_k = diag([sensParams.var_acc;
                sensParams.var_mag]);
    
    
    ASPKF_opt2.use_acc = 1;
end
...
for ii = 1:L
    ...
    %make sigma points
    Sigma_pts_k_1(:,ii+1) = Sigma_pts_k_1(:,1) + sqrt(L+ASPKF_opt2.kappa)*S(:,ii);
    Sigma_pts_k_1(:,L+ii+1) = Sigma_pts_k_1(:,1) - sqrt(L+ASPKF_opt2.kappa)*S(:,ii);
...
lambda = ASPKF_opt2.alpha^2*(L-ASPKF_opt2.kappa)-L;

%find state and covariance predictions
X_k_m = 1/(L+lambda)*(lambda*Sigma_pts_k_m(1:6,1)+0.5*sum(Sigma_pts_k_m(1:6,2:2*L+1),2));

P_k_m = (lambda/(L+lambda)+(1-ASPKF_opt2.alpha^2+ASPKF_opt2.beta))*((Sigma_pts_k_m(1:6,1)-X_k_m(1:6))*(Sigma_pts_k_m(1:6,1)-X_k_m(1:6))');
for ii = 2:2*L+1
...
V_k = (lambda/(L+lambda)+(1-ASPKF_opt2.alpha^2+ASPKF_opt2.beta))*(Sigma_Y(:,1)-y_k_hat)*(Sigma_Y(:,1)-y_k_hat)';
for ii = 2:2*L+1
    V_k = V_k+.5/(L+lambda)*(Sigma_Y(:,ii)-y_k_hat)*(Sigma_Y(:,ii)-y_k_hat)';
end
% V_k = V_k + R_k;

U_k = (lambda/(L+lambda)+(1-ASPKF_opt2.alpha^2+ASPKF_opt2.beta))*(Sigma_pts_k_m(1:6,1)-X_k_m(1:6))*(Sigma_Y(:,1)-y_k_hat)';
...
% update 
ASPKF_opt2.X_hat.bias_gyr = X_k_m(4:6) + DX_k(4:6);

ASPKF_opt2.X_hat.omega_hat = u_b_gyr - ASPKF_opt2.X_hat.bias_gyr; %ang vel is just gyro minus bias
...
ASPKF_opt2.X_hat.q_hat = quatmultiply(q_k_upd,q_k_m_sig(:,1));
...
ASPKF_opt2.P_hat = 0.5*(P_hat + P_hat');
