function [EKF_att] = EKF_att_classic(Sensor, EKF_att, EKF_pos, sensParams, tStep)
...
%update states
EKF_att.X_hat.q_hat = q_k_m + K_k(1:4,:)*(y_k-y_k_hat);

EKF_att.X_hat.q_hat = EKF_att.X_hat.q_hat/norm(EKF_att.X_hat.q_hat,2); % renormalize

EKF_att.X_hat.bias_gyr = bias_gyr_k_m + K_k(5:7,:)*(y_k-y_k_hat);
