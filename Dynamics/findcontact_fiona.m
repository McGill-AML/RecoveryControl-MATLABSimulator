...
%           contactPoint = BUMP_RADII(iBumper)*cos((beta(1)+beta(2))/2)*bumperTangentWorld + BUMP_RADII(iBumper)*sin((beta(1)+beta(2))/2)*bumperCrossProduct + bumperCenterWorld;
%             contactAxisWorld = contactPoint - bumperCenterWorld;
%           contactAxisWorldUnit = contactAxisWorld/norm(contactAxisWorld);
            ...
            Contact.point.contactBody(:,iBumper) = BUMP_POSNS(:,iBumper) + BUMP_RADII(iBumper)*contactAxisBody*sign(bumperCenterWorld(1)^2+bumperCenterWorld(2)^2-poleRadius^2);
            Contact.point.contactWorld(:,iBumper) = real(rotMat'*Contact.point.contactBody(:,iBumper) + state(7:9));
            
            if norm(Contact.point.contactWorld(1:2,iBumper))>=poleRadius
                Contact.point.contactBody(:,iBumper) = BUMP_POSNS(:,iBumper) - BUMP_RADII(iBumper)*contactAxisBody*sign(bumperCenterWorld(1)^2+bumperCenterWorld(2)^2-poleRadius^2);
                Contact.point.contactWorld(:,iBumper) = real(rotMat'*Contact.point.contactBody(:,iBumper) + state(7:9));
            end
            
            % Checking if the generated contact point and the bumper centre
            % are on the opposite sides of the perpendiculaur from poleaxis
            % to tackle the error of contact point jump (in 3D)
            vectorPoleAxis2Contact = Contact.point.contactWorld(1:2,iBumper);
            vectorBumperCenter2Contact = Contact.point.contactWorld(1:2,iBumper) - bumperCenterWorld(1:2);
            angleSign = sign(dot(vectorPoleAxis2Contact,vectorBumperCenter2Contact));
            
         %   if angleSign>0
                Contact.point.contactWorld(:,iBumper) = (Contact.point.contactWorld(:,iBumper)+ptsIntersection(4:6,iBumper)+2*ptsIntersection(1:3,iBumper))/4;
                
                
% %                 disp('angleSign>0')
%                 contactAxisWorld1 = ptsIntersection(1:3,iBumper) - bumperCenterWorld;
%                 contactAxisBody1 = rotMat*contactAxisWorld1;
%                 contactAxisBody1 = contactAxisBody1/norm(contactAxisBody1);
%                 ptsIntersectionBody(1:3,iBumper) = BUMP_POSNS(:,iBumper) + BUMP_RADII(iBumper)*contactAxisBody1*sign(bumperCenterWorld(1)^2+bumperCenterWorld(2)^2-poleRadius^2);
%                 ptsIntersectionWorld(1:3,iBumper) = real(rotMat'*ptsIntersectionBody(1:3,iBumper) + state(7:9))
%                 
%                 contactAxisWorld1 = ptsIntersection(4:6,iBumper) - bumperCenterWorld;
%                 contactAxisBody1 = rotMat*contactAxisWorld1;
%                 contactAxisBody1 = contactAxisBody1/norm(contactAxisBody1);
%                 ptsIntersectionBody(4:6,iBumper) = BUMP_POSNS(:,iBumper) + BUMP_RADII(iBumper)*contactAxisBody1*sign(bumperCenterWorld(1)^2+bumperCenterWorld(2)^2-poleRadius^2);
%                 ptsIntersectionWorld(4:6,iBumper) = real(rotMat'*ptsIntersectionBody(4:6,iBumper) + state(7:9))
%                 
%                 Contact.point.contactWorld(:,iBumper) = (Contact.point.contactWorld(:,iBumper)+ptsIntersectionWorld(4:6,iBumper)+ptsIntersectionWorld(1:3,iBumper))/3;
          %  end
                % Find deflection
...
%             disp('4 points');
            ...
            contactPointBody  = (contactAxisBody1+contactAxisBody2)/2;
%             contactAxisBody1 = contactAxisBody1/norm(contactAxisBody1);
%             contactAxisBody2 = contactAxisBody2/norm(contactAxisBody2);
            ...
            Contact.point.contactBody(:,iBumper) = BUMP_POSNS(:,iBumper) + contactPointBody; %(BUMP_RADII(iBumper)*contactAxisBody1 + BUMP_RADII(iBumper)*contactAxisBody2)/2;
            Contact.point.contactWorld(:,iBumper) = real(rotMat'*Contact.point.contactBody(:,iBumper) + state(7:9));
