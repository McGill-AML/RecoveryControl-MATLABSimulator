...
for iBumper = 1:4
    ...
    % Compute normal for each bumper from horizontal components of thruster
    % location relative to origin in world frame. 
    ...
    anglesIntersection(1,iBumper) = -log((wallLoc - bumperCenterWorld(1) + (bumperCenterWorld(1)^2 - 2*bumperCenterWorld(1)*wallLoc ...
                                    - BUMP_RADII(iBumper)^2*bumperTangentWorld(1)^2 - BUMP_RADII(iBumper)^2*bumperCrossProduct(1)^2 + wallLoc^2)^(1/2)) ...
                                    /(BUMP_RADII(iBumper)*(bumperTangentWorld(1) - bumperCrossProduct(1)*1i)))*1i;
                                
    anglesIntersection(2,iBumper) = -log(-(bumperCenterWorld(1) - wallLoc + (bumperCenterWorld(1)^2 - 2*bumperCenterWorld(1)*wallLoc ...
                                    - BUMP_RADII(iBumper)^2*bumperTangentWorld(1)^2 - BUMP_RADII(iBumper)^2*bumperCrossProduct(1)^2 + wallLoc^2)^(1/2)) ...
                                    /(BUMP_RADII(iBumper)*(bumperTangentWorld(1) - bumperCrossProduct(1)*1i)))*1i;
    ...
    if abs(imag(anglesIntersection(1,iBumper))) <= 1e-5
        ...
        if anglesIntersection(1,iBumper) == anglesIntersection(2,iBumper) %1 pt of intersection
            ...
            % Find points of intersection, eq 2.15 in Fiona's thesis
            ...
            % compute location of P_C, point of contact in body frame
            ...
            % Find deflection
 % edit this to defl = pc*e_n - radius of pole
