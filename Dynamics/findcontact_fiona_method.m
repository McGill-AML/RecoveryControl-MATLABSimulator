...
%         poleTangent = cross(poleVertical,poleAxisToBumperCenterNormal)/norm(cross(poleVertical,poleAxisToBumperCenterNormal)); % RH rule
        ...
        elseif length(beta) == 2
            ...
            contactAxisWorld = contactAxisWorld/norm(contactAxisWorld);
%             contactAxisBody = rotMat*contactAxisWorld;
%             contactAxisBody = contactAxisBody/norm(contactAxisBody);

            if norm([bumperCenterWorld(1) bumperCenterWorld(2)]) <= poleRadius
                Contact.point.contactWorld(:,iBumper) = bumperCenterWorld - BUMP_RADII(iBumper)*contactAxisWorld;
            else
                Contact.point.contactWorld(:,iBumper) = bumperCenterWorld + BUMP_RADII(iBumper)*contactAxisWorld;
            end
            % If the bumper center is within the radius of the pole, the
            % deflection point needs to be computed on the opposite side of
            % the bumper from the intersection points.
%             if norm([bumperCenterWorld(1) bumperCenterWorld(2)]) <= poleRadius
...
%             contactAxisBody2 = contactAxisBody2/norm(contactAxisBody2);
            ...
            % this averages all four intersection points to find a point on
            % the interior of the bumper as the deflection point.
            Contact.point.contactBody(:,iBumper) = BUMP_POSNS(:,iBumper) + ...
                                                    rotMat*(ptsIntersection(1:3,iBumper) + ...
                                                            ptsIntersection(4:6,iBumper) + ...
                                                            point3 + point4)/4;
