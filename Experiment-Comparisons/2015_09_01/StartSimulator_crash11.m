...
%%Crash 6, pitch = 13 to get similar response, wall removed after first
%%impact.
% e = 0.9; 
% Vc_des = 1.05; %1.15
% recover_control = 0;
...
% pitch0 = deg2rad(13);
% z_des = 1.8;%1.5;
% ax_given = 2.776; %0.821748; 13: 1.76, 
% tfinal = 2;
...
% pitch0 = deg2rad(6.3);
% z_des = 1.65;%1.5;
% ax_given = 1.495; %0.821748; 13: 1.76, 
% tfinal = 1.2;
...
% Vc_des = 1.4; %1.98
% recover_control = 0;
% traj_head = -pi + deg2rad(5);
% roll0 = deg2rad(-6);
% pitch0 = deg2rad(23.7);
% z_start = 0.6;
% z_des = 2.1; %roll0 = -6: 2.1
% ax_given = 3.995; %roll0 = -6: 3.995, roll0 = 6: 4.891
% tfinal = 1.2;
...
prop_speed_chkpt = zeros(4,3);
prop_speed_chkpt_flag = zeros(1,3);
...
%     display(size(ttotal))
    ...
    if recover*recover_control == 1
        ...
        [control,ez,evz,eroll,epitch,eyaw,er,omega,roll,pitch,yaw,roll_des,pitch_des,r_des,u1,u2,u3,u4] = ControllerICRA_crash11(Xtotal(end,:),i,t0,dt,z_des,traj_head,traj_att,ez_prev,evz_prev,eroll_prev,epitch_prev,eyaw_prev,er_prev,omega_prev,recover,off_control, Tc_act);
%         if i >= Tc_act
%             u1 = -m*g;
%         end
%         
...
%     end
    ...
    [t,X] = ode45(@(t, X) SpiriMotion_4Circles(t,X,control,wall_loc,wall_plane,e,prop_speed),[i i+dt],x0_step,options);
    ...
    for j = 1:size(X,1)
        [dx,defl1_fine,Fc1,pc1,defl1_rate,defl2_fine,Fc2,pc2,defl2_rate,defl3_fine,Fc3,pc3,defl3_rate,defl4_fine,Fc4,pc4,defl4_rate,prop_speed] = SpiriMotion_4Circles(t(j),X(j,:),control,wall_loc,wall_plane,e,prop_speed);
