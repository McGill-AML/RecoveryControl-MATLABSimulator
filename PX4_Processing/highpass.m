...
hpFilt = designfilt('highpassfir','StopbandFrequency',0.05, ...
         'PassbandFrequency',0.10,'PassbandRipple',0.1, ...
...
figure;plot(TIME(1:index_end),[out_gyr_x, out_gyr_y, out_gyr_z]); grid on;
title('HP filtered Gyroscope'); 

figure;plot(TIME(1:index_end),[out_acc_x, out_acc_y, out_acc_z]); grid on;
title('HP filtered accelerometer'); 

figure;plot(TIME(1:index_end),[out_mag_x, out_mag_y, out_mag_z]); grid on;
title('HP filtered magnetometer'); 


figure;plot(TIME, [LPOS_X, LPOS_Y, LPOS_Z]);grid on;
